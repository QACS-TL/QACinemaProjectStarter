...
import { GETFILMSBYSTATUS } from "../../js/constants/urls";
...
const Schedule = () => {
  ...
  useEffect(() => {
    const getCurrentFilms = async () => {
      try {
        const fetchedCurrentFilms = await axios.get(
          `${GETFILMSBYSTATUS}/${current.status}`
          );
    ...
    const getFutureFilms = async () => {
      try {
        const fetchedFutureFilms = await axios.get(
          `${GETFILMSBYSTATUS}/${future.status}`
        );
